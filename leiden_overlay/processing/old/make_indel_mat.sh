...
while read p; do
  echo "GENE: $p"
  ./new_proc.sh $p
  python new_compile_all_genes.py $p
done < good_genes.txt

# Stage 2: Concatenate all these matrix files into one large and complete matrix.
echo > NEW_big_mat.txt
cat ./EXTENSION/mat_dat/ALL_DYSF.mat | head -1 > NEW_big_mat.txt # adds in the column names
for f in ./EXTENSION/mat_dat/*; do
  echo "processing $f"
  cat $f | tail -n+2 >> NEW_big_mat.txt
done
